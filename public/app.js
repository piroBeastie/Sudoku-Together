...
el.difficultyPicker.addEventListener('click', (e) => {
  ...
  for (const b of el.difficultyPicker.children) {
...
el.playBtn.addEventListener('click', () => {
  ...
  if (name) connect({ name, mode: 'play', difficulty: state.difficulty });
});
...
el.codeForm.addEventListener('submit', (e) => {
  ...
  if (name) connect({ name, mode: 'code', code });
});
...
if (/^[A-Z0-9]{5}$/.test(hashCode)) {
  ...
  if (el.nameInput.value) {
    connect({ name: el.nameInput.value, mode: 'code', code: hashCode });
  } else {
    el.nameInput.focus();
